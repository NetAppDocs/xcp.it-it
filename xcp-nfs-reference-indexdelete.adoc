...
[role="lead"]
Il protocollo NFS `indexdelete` il comando elimina gli indici del catalogo.
...
----
====
La seguente tabella elenca `indexdelete` parametri e loro descrizione.
...
| <<nfs_indexdelete_match,indexdelete -match   >> | Elabora solo i file e le directory corrispondenti al filtro. 


| <<nfs_indexdelete_loglevel,indexdelete -loglevel   >> | Consente di impostare il livello di registro; i livelli disponibili sono INFO, DEBUG (valore predefinito: INFO). 
|===


== indexdelete -corrisponde a <filter>

Utilizzare `-match <filter>` con il `indexdelete` comando per elaborare solo i file e le directory che corrispondono al filtro.
...
Utilizzare `-loglevel <name>` con il `indexdelete` Comando per impostare il livello di log; i livelli disponibili sono INFO e DEBUG. Il livello predefinito è INFO.
