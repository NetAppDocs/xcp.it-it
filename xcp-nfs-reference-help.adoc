...
[role="lead"]
Il protocollo NFS `help` comando visualizza un elenco di comandi, parametri di comando e una breve descrizione di ciascuno di essi. Il `help` Command è utile per i principianti che non hanno familiarità con lo strumento XCP.
...
Utilizzare `info` con il `help` comando per visualizzare la documentazione, gli esempi e le raccomandazioni di sintonizzazione.
